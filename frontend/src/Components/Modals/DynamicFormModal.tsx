...
function convertDateToISO(date: any) {
...
const DynamicFormModal = ({
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      ...
      const defaultFormData = fields.reduce(async (acc: any, field: any) => {
  ...
  const handleChange = (name: any, value: any, regex: any, errorMessage: any) => {
 //   setFormData((prev) => ({ ...prev, [name]: value }));
  ...
  const renderField = ({
    ...
    const commonProps = {
      ...
      onChange: (e: any) => handleChange(name, e.target.value, regex, errorMessage),
      ...
      case 'select':
        return (
          ...
          //{name: "", value: ""}
            ...
            value={options.find((o: any) => o.value === getValue(formData, name)) ?? options[0]}
